...
func (c *CompleteCache) prepareCacheDir2(cmdContext *CmdContext) string {
	basePath, err := filepath.Abs(cmdContext.ConfigPath)
	c.logger.Debug(c.logTag, "config path '%s', abs '%s'", cmdContext.ConfigPath, basePath)
	if err != nil {
		c.logger.Debug(c.logTag, "can't get absolute path of '%s', err: %v", cmdContext.ConfigPath, err)
		return ""
	}
